...
class EvSmartChargePanel extends HTMLElement {
  ...
  _refreshSetup() {
    ...
    if (summary) summary.innerHTML = `Status: <strong>${this._escape(this._state("sensor", "setup_status"))}</strong> · Test: <strong>${this._escape(this._state("sensor", "connection_test_status"))}</strong><br>${this._escape(this._state("sensor", "connection_test_reason", "Nog niet getest"))}`;
  }
  ...
  _refresh() {
    ...
    if (warning) {
      ...
      warning.innerHTML = "<strong>Integratie nog niet gekoppeld</strong><br>Open Apparaat en sensoren en kies de juiste bron-entiteiten.";
    }
    const mode = this._setupConfiguration().control_mode || "monitor";
